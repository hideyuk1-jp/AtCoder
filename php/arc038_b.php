...
function dfs($from = 0, $d = 0)
{
    ...
    if (count($g[$from]) === 0) { //動かせない時
        if ($d % 2) return 'First'; // Secondの番
        else return 'Second'; // Firstの番
    }

    if (isset($memo[$from][$d % 2])) return $memo[$from][$d % 2];

    $cnt = [];
    foreach ($g[$from] as $to => $w) $cnt[dfs($to, $d + 1)] = true;
    if ($d % 2) { // Secondの番
        // Secondが勝つ選択肢があれば勝てる
