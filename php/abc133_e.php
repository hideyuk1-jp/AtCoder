...
function dfs($v = 0)
{
    ...
    if ($v === 0) {
        if (is_null($g[$v])) return $k;
        if ($k - 1 < count($g[$v])) return 0;
        $res = modMul($k, nPr2($k - 1, count($g[$v])));
    } else {
        if (is_null($g[$v])) return 1;
        if ($k - 2 < count($g[$v]) - 1) return 0;
        $res = nPr2($k - 2, count($g[$v]) - 1);
    }
    foreach ($g[$v] as $to) {
        if ($seen[$to]) continue;
        $res = modMul($res, dfs($to));
    }
    return $res;
}
function ints()
